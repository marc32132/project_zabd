...
document.getElementById("logoutLink").onclick = function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const make = document.querySelector('#addyear')

    document.querySelector("#showSt1").addEventListener("click", ev => {
        ev.preventDefault();
        showTable.classList.add("hidden");
        addTable.classList.remove("hidden");
        make.classList.add("hidden");
    });
    document.querySelector("#showSt2").addEventListener("click", ev => {
        ev.preventDefault();
        showTable.classList.add("hidden");
    ...
    document.querySelector("#addSt1").addEventListener("click", ev => {
        ev.preventDefault();
        showTable.classList.remove("hidden");
        addTable.classList.add("hidden");
        make.classList.add("hidden");
    });
    document.querySelector("#addSt2").addEventListener("click", ev => {
        ev.preventDefault();
        showTable.classList.add("hidden");
        addTable.classList.add("hidden");
        make.classList.remove("hidden");
    ...
    document.querySelector("#makeSt1").addEventListener("click", ev => {
        ev.preventDefault();
        showTable.classList.remove("hidden");
        addTable.classList.add("hidden");
        make.classList.add("hidden");
    });
    document.querySelector("#makeSt2").addEventListener("click", ev => {
        ev.preventDefault();
        showTable.classList.remove("hidden");
        addTable.classList.add("hidden");
        make.classList.add("hidden");
  ...
  fetch("/yearGroup/allyears")
  .then(function(response){
      return response.json();
  })
  .then(async function(years){
            
        let out2 ="";
            
        let i = 0;
        let j=0;
        let placeholder2 = document.querySelector("#yearDisplay");
        let placeholder3 = document.querySelector("#yearDisplay2");
        let name;
        let surname;
        let x;
        for(let year of years){
            ...
            let out5 ="";
                ...
                <tr id="add_${j}" class="collapse partic"><td colspan="3">
                    <table style="margin-top: 5px;"><tr><th colspan="5">Add Participants</th></tr>
                    <tr>
                    <th>userName</th>
                    <th>name</th>
                    <th>surname</th>
